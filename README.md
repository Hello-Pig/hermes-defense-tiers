...
| 参数 | 默认值 | 我们设的值 | 设计意图 |
|------|-------|----------|---------|
| `RestartSec` | 100ms | **10s** | 快了可能来不及释放端口，慢了用户等太久 |
...
```
Gateway crash → 10s 后 systemd 自动重启 → 用户无感
```
...
| 维度 | 我们的实现 |
|------|-----------|
| **反馈速度** | systemd 10s 重启 + Watchdog 120s 检测 → 最快 10s 恢复 |
| **冗余层数** | 3 层独立防线，无单点 |
